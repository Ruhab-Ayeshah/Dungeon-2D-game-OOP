...
void Player::Move(char n, Map *l)
{
    ...
    Vector2 golemTiles[2];
    int golemCount = 0;

    if (globalGolem)
    {
        golemTiles[golemCount++] = {
...
{
    golemTiles[golemCount++] = {
        (float)(int)(globalGolem2->Position.x / 32),
        (float)(int)(globalGolem2->Position.y / 32)};
}

// Check against all golem tiles
for (int i = 0; i < golemCount; ++i)
{
    if (next.x == golemTiles[i].x && next.y == golemTiles[i].y)
    {
        return; // Block movement into golem tile
    }
}

// Only move if all tiles passed
